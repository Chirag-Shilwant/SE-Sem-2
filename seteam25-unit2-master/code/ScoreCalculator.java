...
//      System.out.println(vals.size());
        ...
        getTotalScore(totalScore, vals, totalFrames, cumulScore,curScore);

        return cumulScore;
    }

    int[] getTotalScore(int totalScore, Vector<Integer> vals, int totalFrames, int[] cumulScore,int[] curScore) {
        int roll;
        int i = 0;
        int penalty = 0;
        ...
        for (int frames = 0; frames < totalFrames; frames++) {
            ...
            if(frames == 2 && curScore[0] == curScore[1] && curScore[0]==0)
            {
               penalty = curScore[2] / 2;
               totalScore = totalScore - penalty;
            }
            else if(frames > 1)
            {
                 penalty = calculatePenalty(frames, curScore);
                totalScore = totalScore - penalty;
    ...
    private int calculatePenalty(int index, int[] curScore) {
        if (curScore[index] == curScore[index - 1] && curScore[index] == 0) 
        {
            int max = curScore[0];
